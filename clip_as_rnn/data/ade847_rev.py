...
#     return image, image_path, target, index
    def __getitem__(self, index):
        ...
        # 获取对应的 mask 文件名
        filename = os.path.splitext(self.images[index])[0] + '.tif'
        mask_path = os.path.join(self.mask_dir, filename)

        try:
            target = np.asarray(Image.open(mask_path), dtype=np.int32) + 1
        except FileNotFoundError:
            print(f"[WARNING] Mask not found for image: {image_path}")
            dummy_mask = np.zeros((image.size[1], image.size[0]), dtype=np.int32)
            target = dummy_mask
