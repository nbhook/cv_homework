...
class CaR(nn.Module):
  ...
  def _forward_car(self, ori_img, text):
    ...
    # # 返回空的结果，防止报错
    #   h, w = im_ori.shape[-2:]
    #   return torch.zeros(0, h, w), torch.zeros(0)
    if len(refined_masks) == 0:
      H, W = im_ori.shape[-2:]
      dummy_mask = torch.zeros((len(text), H, W), device=im_ori.device)
      dummy_score = torch.zeros((len(text),), device=im_ori.device)
      return dummy_mask, dummy_score
